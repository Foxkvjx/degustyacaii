...
begin
  ...
  avatar := nullif(trim(coalesce(new.raw_user_meta_data->>'avatar_url','')), '');
  insert into public.profiles(id,full_name,avatar_url) values(new.id,display_name,avatar)
...
begin
  ...
  if new.user_id is null then raise exception 'Usuário autenticado obrigatório'; end if;
  return new;
end; $$;
...
declare t text;
begin
  foreach t in array array['produtos','gastos','vendas','receitas','receita_itens','estoque_movimentacoes','work_calendar_events'] loop
    execute format('drop trigger if exists set_record_owner on public.%I',t);
    execute format('create trigger set_record_owner before insert on public.%I for each row execute function public.set_record_owner()',t);
    execute format('alter table public.%I enable row level security',t);
    execute format('drop policy if exists owner_isolation on public.%I',t);
    execute format('create policy owner_isolation on public.%I as restrictive for all to authenticated using (user_id=auth.uid()) with check (user_id=auth.uid())',t);
