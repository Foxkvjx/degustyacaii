...
import { CalendarDays, ChevronLeft, ChevronRight, CloudRain, Download, ExternalLink, MapPin, RefreshCw, Sun, ThermometerSun, Umbrella } from "lucide-react";
...
const pretty = (s: string) => new Date(`${s}T12:00:00`).toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" });
...
export default function Calendario() {
  ...
  function exportIcs() {
    const body = events.map(e => `BEGIN:VEVENT\nUID:${e.id}@degustyacai\nDTSTAMP:${new Date().toISOString().replace(/[-:]/g,"").replace(/\\.\\d{3}/,"")}\nDTSTART:${e.work_date.replace(/-/g,"")}T100000\nDTEND:${e.work_date.replace(/-/g,"")}T180000\nSUMMARY:${e.title}\nLOCATION:${e.location ?? ""}\nEND:VEVENT`).join("\n");
  ...
  return <main className="min-h-screen bg-slate-50 pb-24 sm:pb-0"><div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8">
    ...
    <header className="mt-4 flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-slate-400">Planejamento</p><h1 className="mt-1 text-3xl font-bold tracking-tight">Calendário</h1><p className="mt-1 text-sm text-slate-500">Escolha os dias de trabalho e veja se o clima ajuda na venda.</p></div><button onClick={loadWeather} disabled={weatherLoading} className="flex h-11 w-11 items-center justify-center rounded-full bg-black text-white dark:bg-white dark:text-black" aria-label="Atualizar previsão"><RefreshCw size={17} className={weatherLoading ? "animate-spin" : ""}/></button></header>
