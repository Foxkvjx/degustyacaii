...
export default function Dashboard() {
  ...
  async function load() {
    ...
    setVendas((sales.data ?? []) as Venda[]); setProdutos((stock.data ?? []) as Produto[]);
  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-24 sm:pb-0">
      <div className="mx-auto max-w-6xl px-5 py-7 sm:px-8 sm:py-10">
        ...
        <section className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">{[["Copos", loading ? "..." : String(cups), "Hoje", ShoppingBag],["Ticket", loading ? "..." : money(ticket), "Médio", CircleDollarSign],["Açaí", acai ? `${Number(acai.qtd)} ${acai.unidade === "litros" ? "L" : acai.unidade}` : "0 L", "Estoque", Package],["Estoque", loading ? "..." : money(stockValue), "Valor", Target]].map(([label,value,note,Icon]) => { const I=Icon as typeof Package; return <div key={String(label)} className="rounded-2xl bg-white p-4 dark:bg-[#080808] sm:p-5"><I size={18} strokeWidth={1.7} className="text-slate-500"/><p className="mt-5 text-[11px] font-medium text-slate-500">{label}</p><p className="mt-1 text-xl font-semibold tracking-[-0.04em] text-slate-900 dark:text-white">{value}</p><p className="mt-1 text-[11px] text-slate-400">{note}</p></div> })}</section>
        <section className="mt-4 rounded-2xl bg-white p-5 dark:bg-[#080808] sm:p-6"><div className="flex items-center justify-between"><div><p className="text-xs font-medium text-slate-500">Desempenho</p><h2 className="mt-1 text-lg font-semibold tracking-[-0.035em] text-slate-900 dark:text-white">Vendas recentes</h2></div><Link href="/vendas" className="flex items-center gap-1 text-xs font-semibold text-slate-500">Ver tudo <ChevronRight size={14}/></Link></div><div className="mt-7 flex h-36 items-end gap-2 sm:h-44 sm:gap-3">{lastDays.map((d,i)=><div key={i} className="flex flex-1 flex-col items-center gap-2"><div className="w-full max-w-10 rounded-t-md bg-black dark:bg-white" style={{height:`${Math.max(5,d.cups/max*100)}%`}}/><span className="text-[10px] text-slate-400">{d.label}</span></div>)}</div></section>
        <section className="mt-4 grid gap-3 sm:grid-cols-2"><Link href="/vendas" className="group rounded-2xl bg-white p-5 dark:bg-[#080808]"><div className="flex items-center justify-between"><div><p className="text-xs text-slate-500">Operação</p><p className="mt-1 font-semibold text-slate-900 dark:text-white">Registrar venda</p></div><span className="flex h-10 w-10 items-center justify-center rounded-full bg-black text-white dark:bg-white dark:text-black"><Plus size={18}/></span></div></Link><Link href="/estoque" className="group rounded-2xl bg-white p-5 dark:bg-[#080808]"><div className="flex items-center justify-between"><div><p className="text-xs text-slate-500">Controle</p><p className="mt-1 font-semibold text-slate-900 dark:text-white">Ver estoque</p></div><span className="flex h-10 w-10 items-center justify-center rounded-full bg-black text-white dark:bg-white dark:text-black"><ArrowUpRight size={18}/></span></div></Link></section>
      </div><MobileNav />
    </main>
  );
}
