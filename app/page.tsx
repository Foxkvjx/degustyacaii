...
const screens = [
  ...
  ["/parametros", "Parâmetros", "Configurações do negócio", "06"],
];
        ...
        <header className="mb-10 sm:mb-14">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-3 max-w-xl text-sm leading-6 text-slate-500 sm:text-base">Estoque, vendas, gastos e clima em um único painel.</p>
            </div>
          </div>
        </header>
        ...
        <section className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {screens.map(([href, title, description, number]) => (
            <Link
              key={href}
              href={href}
              className="group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition duration-200 active:scale-[0.99] sm:p-6 sm:hover:-translate-y-0.5 sm:hover:shadow-md"
            >
              <div className="flex items-start justify-between gap-4">
                <span className="text-xs font-medium tabular-nums text-slate-400">{number}</span>
                <span className="text-sm text-slate-400 transition-transform group-hover:translate-x-0.5">↗</span>
              </div>
